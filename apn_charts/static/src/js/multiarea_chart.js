...
export class MultiAreaChart extends Component {
    static template = "apn_charts.MultiAreaChartTemplate"
    ...
    _renderChart() {
        ...
        // Construir datasets para Chart.js con área
        const chartDatasets = datasets.map((ds, index) => {
            const color = ds.color || this._getDefaultColor(index)
            return {
                label: ds.label || `Dataset ${index + 1}`,
                data: ds.values,
                borderColor: color,
                backgroundColor: color + '15', // 15 = ~8% opacidad
                borderWidth: 2.5,
...
MultiAreaChart.template = "apn_charts.MultiAreaChartTemplate"
registry.category("components").add("MultiAreaChart", MultiAreaChart)
registry.category("fields").add("multi_area_chart", { component: MultiAreaChart })
